...
{
    int counter = (int)log((double) index);
    if(counter % 2 == 0)    return MIN_LAYER;
    return MAX_LAYER;
}
*/
#define MAX(x, y)   \
...
//                sift_up_min(heap, index);
        ...
        else
        {
            ...
            LOG("sift_up_MAX_LAYER %d and %d, res = %d\n", get_ptr(heap, index)->value, get_ptr(heap, parent)->value, cmp_elem_heap(get_ptr(heap, index), get_ptr(heap, parent)));
...
//        sift_down_max(heap, parent);
    ...
    if(ind_min != index)
    {
        LOG("sift_down_min swap %d and %d\n", get_ptr(heap, index)->value, get_ptr(heap, ind_min)->value);
        swap(get_ptr(heap, index), get_ptr(heap, ind_min));
        if(is_son)
        {
            sift_down_max(heap, ind_min);
//            sift_down_min(heap, index);//
        }
        else
        {
            sift_down_min(heap, ind_min);
        }
    }
}
...
int extractMin(BinHeap *heap)
{
    ...
    {
        (get_ptr(heap, 0)->value) = INF_VAL_MIN;
    }
...
int extractMax(BinHeap *heap)
{
    ...
    if(type == MIN_LAYER)
    {
        (get_ptr(heap, index_max)->value) = INF_VAL_MAX;
    }
    else
    {
        (get_ptr(heap, index_max)->value) = INF_VAL_MIN;
    }
...
void init_BinHeap(BinHeap *heap, int size)
{
    ...
    for(int i = 0; i < size; i++)
    {
        ...
        {
            (get_ptr(heap, i)->value) = INF_VAL_MIN;
        }
    }
    heap->size = 0;
}
int main()
{
    ...
    for(int i = 0; i < amount_op; i++)
    {
        ...
        {
            val = getMin(&heap);
            if (val == INF_VAL_MAX) printf("error\n");
            else
            {
                printf("%d\n", val);
            }
